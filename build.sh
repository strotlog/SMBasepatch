...
for subdir in $(ls -d vanilla romhacks/*); do
    ...
    popd > /dev/null
    python3 resources/create_ips.py build/00.sfc build/ff.sfc build/$subdir/multiworld-basepatch.ips
    echo Built: build/$subdir/multiworld-basepatch.ips
    rm build/00.sfc build/ff.sfc

    python3 resources/sym2json.py build/$subdir/multiworld.sym common/*.asm > build/$subdir/sm-basepatch-symbols.json
    ...
    if [ -f $subdir/variapatches.includelist ]; then
        ...
        for ipsref in $(cat $subdir/variapatches.includelist | grep -v "^#" | grep -v "^\/\/" | grep -v "^;"); do
            ...
            echo -n " "
            python3 resources/merge_ips.py $ipsfile build/$subdir/variapatches.ips
        done
